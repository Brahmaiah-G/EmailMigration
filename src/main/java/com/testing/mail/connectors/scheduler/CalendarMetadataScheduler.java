...
import com.testing.mail.connectors.management.CalendarMetadataTask;
...
public class CalendarMetadataScheduler {
	...
	private void processWorkSpaes(final ThreadControl threadControl,CalendarMoveQueue calMoveQueue) {
		...
		calMoveQueue.setProcessStatus(com.testing.mail.repo.entities.PROCESS.IN_PROGRESS);
	...
	private List<CalendarMoveQueue> getDistrinctMoveWorkspaceIds(int pageNo) {
		...
		return dbConnectorService.getEmailQueueRepoImpl().findCalendarMoveQueueByProcessStatusWithPazination(Arrays.asList(com.testing.mail.repo.entities.PROCESS.NOT_PROCESSED,com.testing.mail.repo.entities.PROCESS.IN_PROGRESS), 20, pageNo);
	}
}
