...
import com.testing.mail.connectors.management.MetadataUpdateTask;
...
public class MetadataUpdateScheduler {
	...
	private void processWorkSpaes(final ThreadControl threadControl,EmailMoveQueue emailMoveQueue,int pageNo) {
		...
		updateMoveQueue(emailMoveQueue.getId(), ExceptionConstants.STARTED,com.testing.mail.repo.entities.PROCESS.IN_PROGRESS);
	...
	private List<EmailMoveQueue> getDistrinctMoveWorkspaceIds(MongoTemplate mongoOps, ThreadControl threadControl,int pageNo) {
		...
		return dbConnectorService.getEmailQueueRepoImpl().findMoveQueueByProcessStatusWithPazination(Arrays.asList(com.testing.mail.repo.entities.PROCESS.IN_PROGRESS), 20, pageNo,CLOUD_NAME.OUTLOOK.name());
	}
	...
	private void updateMoveQueue(String moveQueueId,String errorDescription,com.testing.mail.repo.entities.PROCESS processStatus) {
