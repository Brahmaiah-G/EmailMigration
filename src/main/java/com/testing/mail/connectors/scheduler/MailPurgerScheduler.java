...
public class MailPurgerScheduler {
	...
	private void processWorkSpaces(ThreadControl threadControl,EmailPurgeQueue emailPurgeQueue) {
		...
		emailPurgeQueue.setProcessStatus(com.testing.mail.repo.entities.PROCESS.IN_PROGRESS);
	...
	private List<EmailPurgeQueue> getDistrinctMoveWorkspaceIds(ThreadControl threadControl) {
		checkThreadPool();
		return dbConnectorService.getEmailQueueRepoImpl().findPurgeQueueByProcessStatusWithPazination(Arrays.asList(com.testing.mail.repo.entities.PROCESS.NOT_PROCESSED,com.testing.mail.repo.entities.PROCESS.IN_PROGRESS), 20, 0);
