 ...
 *@see com.cloudfuze.mail.connectors.scheduler.CalendarPickerScheduler &#64;CalendarPickerScheduler
 */
			...
			for(MailChanges changes : mailChanges) {
				...
				if(changes!=null && changes.getSourceId()!=null) {
					...
					if(info!=null) {
						...
						eventInstacesInfo.setProcessStatus(com.cloudfuze.mail.repo.entities.PROCESS.NOT_PROCESSED);
	...
	private void checkInstances(String recurenceId,CalenderInfo eventsInfo,String endTime) {
		...
		if(null!=emails && !emails.isEmpty()) {
			...
			for(CalenderInfo emailFlagsInfo : emails) {
				...
				target.setProcessStatus(com.cloudfuze.mail.repo.entities.PROCESS.valueOf(_emailInfo.getProcessStatus().name()));
