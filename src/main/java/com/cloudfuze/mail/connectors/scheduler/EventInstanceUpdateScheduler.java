...
import com.cloudfuze.mail.connectors.management.EventInstanceUpdationTask;
...
public class EventInstanceUpdateScheduler {
	...
	private void processWorkSpaes(final ThreadControl threadControl,CalendarMoveQueue calendarMoveQueue,int pageNo,List<CalendarMoveQueue> emailWorkSpaeIds ) {
		...
		calendarMoveQueue.setProcessStatus(com.cloudfuze.mail.repo.entities.PROCESS.IN_PROGRESS);
		...
		Query query = new Query(Criteria.where(DBConstants.EMAILWORKSPACEID).is(calendarMoveQueue.getEmailWorkSpaceId()).and(DBConstants.PROCESS_STATUS).is(com.cloudfuze.mail.repo.entities.PROCESS.INSTANCE_NOTPROCESSED).and("preScan").is(false).and(DBConstants.RETRY).lte(threadControl.getRetryCount())).limit((int)count);
	...
	private List<CalendarMoveQueue> getDistrinctMoveWorkspaceIds(int pageNo) {
		return dbConnectorService.getEmailQueueRepoImpl().findCalendarMoveQueueByProcessStatusWithPazination(Arrays.asList(com.cloudfuze.mail.repo.entities.PROCESS.NOT_PROCESSED,com.cloudfuze.mail.repo.entities.PROCESS.IN_PROGRESS), 20, pageNo,CLOUD_NAME.OUTLOOK.name());
	}
