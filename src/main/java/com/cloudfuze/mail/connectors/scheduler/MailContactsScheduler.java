...
import com.cloudfuze.mail.connectors.management.ContactsMigrationTask;
...
@Slf4j
//@Component
public class MailContactsScheduler {
	...
	public MailContactsScheduler() {
	}
	public MailContactsScheduler(ThreadPoolTaskExecutor threadPoolTaskExecutor) {
	...
	private void processWorkSpaes(final ThreadControl threadControl,EmailMoveQueue emailMoveQueue,int pageNo,List<EmailMoveQueue> emailWorkSpaeIds) {
		...
		List<ContactsInfo> emails = extractRulesBasedOnPrior(threadControl, emailMoveQueue.getEmailWorkSpaceId(), count);
		...
		for(ContactsInfo contact : emails) {
			if(!checkThreadPool()) {
				return;
			}
			contact.setProcessStatus(PROCESS.IN_PROGRESS);
			contact.setThreadBy(Thread.currentThread().getName());
			contact.setErrorDescription(null);
			dbConnectorService.getEmailInfoRepoImpl().saveContact(contact);
			threadPoolTaskExecutor.submit(new ContactsMigrationTask(emailWorkSpace, mailServiceFactory, dbConnectorService, contact));
		}
		pageNo = pageNo+20;
	...
	private List<ContactsInfo> extractRulesBasedOnPrior(final ThreadControl threadControl, String emailWorkSpaceId, long count) {
		...
		return mongoTemplate.find(query.limit((int) count), ContactsInfo.class);
	}
	...
	private List<EmailMoveQueue> getDistrinctMoveWorkspaceIds(MongoTemplate mongoOps, ThreadControl threadControl,int pageNo) {
		...
		return dbConnectorService.getEmailQueueRepoImpl().findMoveQueueByforContacts(Arrays.asList(PROCESS.NOT_PROCESSED,PROCESS.IN_PROGRESS), 20, pageNo);
	}
