...
import com.cloudfuze.mail.connectors.management.MailDraftsCreationTask;
...
@Slf4j
public class MailDraftsCreationcheduler {
	...
	public MailDraftsCreationcheduler() {
	}
	public MailDraftsCreationcheduler(ThreadPoolTaskExecutor threadPoolTaskExecutor) {
	...
	private void processWorkSpaes(final ThreadControl threadControl,EmailMoveQueue emailMoveQueue,int pageNo,List<EmailMoveQueue> emailWorkSpaeIds) {
		EmailWorkSpace emailWorkSpace = dbConnectorService.getWorkSpaceRepoImpl().findOne("6572e46a38e8f83610f115b0");
		...
		//Query query = new Query(Criteria.where(DBConstants.EMAILWORKSPACEID).is(emailMoveQueue.getEmailWorkSpaceId()).and(DBConstants.TO_CLOUD).is(Clouds.CLOUD_NAME.OUTLOOK.name()).and(DBConstants.PROCESS_STATUS).is(PROCESS.NOT_PROCESSED).and("preScan").is(false).and("retryCount").lt(threadControl.getRetryCount())).limit((int)count);
		Query query = new Query(Criteria.where("_id").is("6572e49e38e8f83610f115e1"));
		...
		while(processFolders) {
			...
			if(!infos.isEmpty()) {
				infos.forEach(emailInfo->{
					if(checkThreadPool()) {
						emailInfo.setProcessStatus(EmailInfo.PROCESS.DRAFT_CREATION_INPROGRESS);
						emailInfo.setThreadBy(Thread.currentThread().getName());
						emailInfo.setErrorDescription(null);
						emailInfo.setDraftBatchId(batchId);
						dbConnectorService.getEmailInfoRepoImpl().save(emailInfo);
						emailInfos.add(emailInfo);
					}
				});
				//threadPoolTaskExecutor.submit(new MailDraftsCreationTask(mailServiceFactory, dbConnectorService, emailWorkSpace, emailInfos));
				new MailDraftsCreationTask(mailServiceFactory, dbConnectorService, emailWorkSpace, emailInfos).run();
	...
	private List<EmailMoveQueue> getDistrinctMoveWorkspaceIds(MongoTemplate mongoOps, ThreadControl threadControl,int pageNo) {
		...
		emailMoveQueue.setEmailWorkSpaceId("");
		return Arrays.asList(emailMoveQueue);
		//return dbConnectorService.getEmailQueueRepoImpl().findMoveQueueByProcessStatusWithPazination(Arrays.asList(PROCESS.PROCESSED), 20, pageNo,CLOUD_NAME.OUTLOOK.name());
	}
