...
import com.cloudfuze.mail.connectors.management.CalendarPickingTask;
...
public class CalendarPickerScheduler {
	...
	private void processWorkSpaces(ThreadControl threadControl,CalendarPickingQueue calendarPickingQueue,List<CalendarPickingQueue> emailWorkSpaeIds,int pageNo) {
		...
		calendarPickingQueue.setProcessStatus(com.cloudfuze.mail.repo.entities.PROCESS.IN_PROGRESS);
	...
	private void checkForPickingCompletion() {
		List<CalendarPickingQueue> inQueues = dbConnectorService.getEmailQueueRepoImpl().findCalendarPickingByProcessStatusWithPazination(Arrays.asList(com.cloudfuze.mail.repo.entities.PROCESS.IN_PROGRESS,com.cloudfuze.mail.repo.entities.PROCESS.CONFLICT), 100, 0);
		if(!inQueues.isEmpty()) {
			inQueues.forEach(pickingQueue->{
				List<CalenderInfo> infos = dbConnectorService.getCalendarInfoRepoImpl().findByProcessStatus(pickingQueue.getEmailWorkSpaceId(), Arrays.asList(com.cloudfuze.mail.repo.entities.PROCESS.IN_PROGRESS,com.cloudfuze.mail.repo.entities.PROCESS.NOT_STARTED,com.cloudfuze.mail.repo.entities.PROCESS.CONFLICT));
				if(infos.isEmpty()) {
					pickingQueue.setProcessStatus(com.cloudfuze.mail.repo.entities.PROCESS.PROCESSED);
					dbConnectorService.getEmailQueueRepoImpl().saveCalendarPickingQueue(pickingQueue);
				}else {
					List<CalenderInfo> _infos = dbConnectorService.getCalendarInfoRepoImpl().findByProcessStatus(pickingQueue.getEmailWorkSpaceId(), Arrays.asList(com.cloudfuze.mail.repo.entities.PROCESS.CONFLICT));
					if(_infos.size()==infos.size()) {
						pickingQueue.setProcessStatus(com.cloudfuze.mail.repo.entities.PROCESS.CONFLICT);
						dbConnectorService.getEmailQueueRepoImpl().saveCalendarPickingQueue(pickingQueue);
					}
				}
			});
